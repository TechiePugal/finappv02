...
export default function Customers(){
  ...
  async function openHistory(c){
    ...
    try{
      ...
      ip.docs.forEach(x=>{const r=x.data();if(lids.has(r.borrowerId)&&['Paid','Partial'].includes(r.status))T.push({date:r.paymentDate||r.month,label:`Interest ${r.status==='Partial'?'(partial) ':''}— ${r.month}`,amount:r.amountPaid||0,type:'interest',parentId:r.borrowerId,remarks:r.remarks||''});});
      lr.docs.forEach(x=>{const r=x.data();if(lids.has(r.borrowerId)&&!r.deleted)T.push({date:r.date,label:'Loan repayment',amount:r.repaidAmount||r.amount||0,type:'repay',parentId:r.borrowerId,remarks:r.remarks||''});});
      dp.docs.forEach(x=>{const r=x.data();if((dids.has(r.depositorId)||dids.has(r.depositId))&&r.status==='Paid')T.push({date:r.paymentDate||r.month,label:`Deposit interest — ${r.month||''}`,amount:r.totalPayout||r.amountPaid||0,type:'depint',parentId:r.depositorId||r.depositId,remarks:r.remarks||''});});
      ec.docs.forEach(x=>{const r=x.data();if(eids.has(r.loanId))T.push({date:r.date,label:`EMI #${r.periodNo||''} ${r.status==='Partial'?'(partial)':''}`,amount:r.amount||0,type:'emipay',parentId:r.loanId,remarks:r.remarks||''});});
