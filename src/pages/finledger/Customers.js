...
export default function Customers(){
  ...
  async function openHistory(c){
    ...
    try{
      ...
      ip.docs.forEach(x=>{const r=x.data();if(lids.has(r.borrowerId)&&['Paid','Partial'].includes(r.status))T.push({date:r.paymentDate||r.month,label:`Interest ${r.status==='Partial'?'(partial) ':''}— ${r.month}`,amount:r.amountPaid||0,type:'interest',parentId:r.borrowerId});});
